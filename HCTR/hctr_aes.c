...
void HCTR_encrypt_1(const unsigned char *pt,
                    unsigned char *ct,
                    const unsigned char *tweak,
                    int pt_len,
                    ae_ctx *ctx);
void HCTR_encrypt_2(const unsigned char *pt,
                    unsigned char *ct,
                    const unsigned char *tweak,
                    int pt_len,
                    ae_ctx *ctx);
void HCTR_encrypt_4(const unsigned char *pt,
                    unsigned char *ct,
                    const unsigned char *tweak,
                    int pt_len,
                    ae_ctx *ctx);
...
{
#ifdef USE_AESNI_1
    HCTR_encrypt_1(pt, ct, nonce, pt_len, ctx);
#endif
#ifdef USE_AESNI_2
    HCTR_encrypt_2(pt, ct, nonce, pt_len, ctx);
#endif
#ifdef USE_AESNI_4
    HCTR_encrypt_4(pt, ct, nonce, pt_len, ctx);
#endif
#ifdef USE_AESNI_8
                    ...
                    ae_ctx *ctx)
{
    ...
    __m128i ctr2;
    __m128i tmp1, tmp2, tweakswap, ciphertext, Z1Z2;
    __m128i H = _mm_loadu_si128((__m128i *)ctx->key2), H2;
    int i = 0;
    ...
    {
        tmp1 = _mm_shuffle_epi8(_mm_loadu_si128(&((__m128i *)pt)[2 * i + 1]), BSWAP_MASK);
        tmp2 = _mm_shuffle_epi8(_mm_loadu_si128(&((__m128i *)pt)[2 * i + 2]), BSWAP_MASK);
        tmp1 = _mm_xor_si128(D, tmp1);
        reduce2(H, H2, tmp2, tmp1, &D);
    }
    for (i = 2 * i + 1; i < pt_len / 16; i++)
    {
    ...
    for (i = 0; i < (pt_len - 16) / 32; i++)
    {
        ...
        tmp1 = _mm_xor_si128(tmp1, _mm_load_si128(&((__m128i *)pt)[i * 2 + 1]));
        tmp2 = _mm_xor_si128(tmp2, _mm_load_si128(&((__m128i *)pt)[i * 2 + 2]));
        _mm_store_si128(&((__m128i *)ct)[i * 2 + 1], tmp1);
        _mm_store_si128(&((__m128i *)ct)[i * 2 + 2], tmp2);
        tmp1 = _mm_shuffle_epi8(tmp1, BSWAP_MASK);
        tmp2 = _mm_shuffle_epi8(tmp2, BSWAP_MASK);
        tmp1 = _mm_xor_si128(hash2, tmp1);
        reduce2(H, H2, tmp2, tmp1, &hash2);
    }
    for (i = 2 * i + 1; i < pt_len / 16; i++)
    {
        tmp1 = _mm_shuffle_epi8(ctr1, BSWAP_EPI64);
                    ...
                    ae_ctx *ctx)
{
    ...
    __m128i ctr1 = _mm_setzero_si128();
    __m128i ctr2, ctr3, ctr4;
    __m128i tmp1, tmp2, tmp3, tmp4, tweakswap, ciphertext, Z1Z2;
    __m128i H = _mm_loadu_si128((__m128i *)ctx->key2), H2, H3, H4;
    int i = 0;
    lenswap = _mm_insert_epi64(lenswap, pt_len * 8, 0);
    H = _mm_shuffle_epi8(H, BSWAP_MASK);
    gfmul1(H, H, &H2);
    ...
    {
        tmp1 = _mm_shuffle_epi8(_mm_loadu_si128(&((__m128i *)pt)[4 * i + 1]), BSWAP_MASK);
        tmp2 = _mm_shuffle_epi8(_mm_loadu_si128(&((__m128i *)pt)[4 * i + 2]), BSWAP_MASK);
        tmp3 = _mm_shuffle_epi8(_mm_loadu_si128(&((__m128i *)pt)[4 * i + 3]), BSWAP_MASK);
        tmp4 = _mm_shuffle_epi8(_mm_loadu_si128(&((__m128i *)pt)[4 * i + 4]), BSWAP_MASK);
        tmp1 = _mm_xor_si128(D, tmp1);
        reduce4(H, H2, H3, H4, tmp4, tmp3, tmp2, tmp1, &D);
    }
    for (i = 4 * i + 1; i < pt_len / 16; i++)
    {
        tmp1 = _mm_load_si128(&((__m128i *)pt)[i]);
        tmp1 = _mm_shuffle_epi8(tmp1, BSWAP_MASK);
        D = _mm_xor_si128(D, tmp1);
    ...
    for (i = 0; i < (pt_len - 16) / 64; i++)
    {
        ...
        tmp1 = _mm_xor_si128(tmp1, _mm_load_si128(&((__m128i *)pt)[i * 4 + 1]));
        tmp2 = _mm_xor_si128(tmp2, _mm_load_si128(&((__m128i *)pt)[i * 4 + 2]));
        tmp3 = _mm_xor_si128(tmp3, _mm_load_si128(&((__m128i *)pt)[i * 4 + 3]));
        tmp4 = _mm_xor_si128(tmp4, _mm_load_si128(&((__m128i *)pt)[i * 4 + 4]));
        _mm_store_si128(&((__m128i *)ct)[i * 4 + 1], tmp1);
        _mm_store_si128(&((__m128i *)ct)[i * 4 + 2], tmp2);
        _mm_store_si128(&((__m128i *)ct)[i * 4 + 3], tmp3);
        _mm_store_si128(&((__m128i *)ct)[i * 4 + 4], tmp4);
        tmp1 = _mm_shuffle_epi8(tmp1, BSWAP_MASK);
        tmp2 = _mm_shuffle_epi8(tmp2, BSWAP_MASK);
        tmp3 = _mm_shuffle_epi8(tmp3, BSWAP_MASK);
        tmp4 = _mm_shuffle_epi8(tmp4, BSWAP_MASK);
        tmp1 = _mm_xor_si128(hash2, tmp1);
        reduce4(H, H2, H3, H4, tmp4, tmp3, tmp2, tmp1, &hash2);
    }
    for (i = 4 * i + 1; i < pt_len / 16; i++)
    {
        tmp1 = _mm_shuffle_epi8(ctr1, BSWAP_EPI64);
